module.exports = function (RED) {
    ...
    function AcronisCtrlNode(config) {
        ...
        this.on('input', function (msg) {
            ...
            if (typeof msg.sn !== 'undefined' && msg.sn !== '')
                config.sn = msg.sn;  
            if (typeof msg.asz !== 'undefined' && msg.asz !== '')
                config.asz = msg.asz;  
            
            var jsonString = getjsoncontentData(config);
            statusNode.status({fill: "red", shape: "ring", text: "sending"});
            options = sethttpOption(url, port, '/webresources/RecoveryMgmt', 'post', username, pwd);
    ...
    RED.nodes.registerType("AcronisCtrl", AcronisCtrlNode);
};